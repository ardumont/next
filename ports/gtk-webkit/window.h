...
gboolean window_send_event(GtkWidget *_widget, GdkEventKey *event, gpointer data) {
	...
	GVariant *key_chord = g_variant_new("(sisas)",
			window->buffer->identifier,
	...
	g_debug("consumed %s", g_variant_print(consumed, TRUE));
	...
	GVariant *no_arg = g_variant_new("()");
	msg = soup_xmlrpc_message_new("http://localhost:8081/RPC2",
			method_name, no_arg, &error);
